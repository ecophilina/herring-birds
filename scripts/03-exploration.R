...
p <- filter(all_dat, group != "Rapter") %>%
  ...
  geom_violin() + 
  geom_jitter(aes(colour = year, alpha = SPCount,
    size = SPCount
    ),  # size =1, 
    # alpha = 0.1, 
    width = 0.25
...
p <- filter(all_dat, group != "Rapter") %>% filter(!is.na(matchID)) %>%
  ggplot(aes(as.factor(year), SPCount, group = year)) +
  # ggplot(aes(as.factor(year), log(SPCount+1), group = period)) +
  geom_violin() + 
  geom_jitter(aes(colour = year, alpha = SPCount,
...
max_counts_surv <- all_dat %>% group_by(year, survey, matchID) %>%
  mutate(surv_sums = sum(SPCount)) %>% ungroup() %>%
  group_by(year, matchID) %>%
  mutate(max_SPCount = max(surv_sums)) %>%
  select(year, matchID, group, max_SPCount, spawn_stage) %>% unique()
...
max_group_surv <- all_dat %>% group_by(year, survey, group) %>%
  mutate(surv_sums = sum(SPCount)) %>% ungroup() %>%
  group_by(year, group) %>%
  mutate(max_GRCount = max(surv_sums)) %>%
  select(year, group, max_GRCount, spawn_stage) %>% unique()
